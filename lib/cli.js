      ...
      prompt.get({
        ...
        function(err, result) {
          ...
          if (result.agree.toLowerCase() !== 'y') return console.log(chalk.red('Package publish aborted by user...'));
	        ...
	        console.log(chalk.green('    executing git fetch --unshallow...'));
	        shell.exec('git fetch --unshallow', function(code) {
		        if (!code || code === 128) { //unshallow error
...
function getPackage(name, callback) {
  ...
  request(options, function(error, response, body) {
    if (!error && (response.statusCode === 200 || response.statusCode === 201)) {
      ...
      console.log('Package not found. ', error, options, response.statusCode, body);
    }
  });
}
